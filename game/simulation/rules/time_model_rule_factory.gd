...
static func create_registry() -> SimulationRuleRegistry:
	...
	registry.register_rule(AdvanceCompetitorsRule.new())
	registry.register_rule(PostOperatingCostRule.new())
	registry.register_rule(PostComputeContractCostsRule.new())
