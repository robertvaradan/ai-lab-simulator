...
func _verify_research_first(core: SimulationCore, state: GameState) -> void:
	...
	var month_one_competitiveness: Vector3i = CodingAgentMarket.technical_competitiveness_vector(
		after_month_one.company.models[STARTING_MODEL_ID].evaluations,
		after_month_one.world.technical_frontier
	)
	...
	var month_two_competitiveness: Vector3i = CodingAgentMarket.technical_competitiveness_vector(
		after_month_two.company.models[STARTING_MODEL_ID].evaluations,
		after_month_two.world.technical_frontier
	)
	_expect(
	...
	if advanced == null or not advanced.has_candidate_state():
		return
	var ended: GameState = advanced.candidate_state
	_expect(ended.cash_ledger.calculate_balance_musd() == 58, "Research-first Cash is incorrect.")
	_expect(_category_total(ended, PROJECT_START_CATEGORY) == -65, "Research-first Project cost is incorrect.")
	...
	_expect(
		ended.company.models.has(RESEARCH_MODEL_ID),
		"Research-first did not release the completed Model."
	)
	if ended.company.models.has(RESEARCH_MODEL_ID):
		var competitiveness: Vector3i = CodingAgentMarket.technical_competitiveness_vector(
			ended.company.models[RESEARCH_MODEL_ID].evaluations,
			ended.world.technical_frontier
		)
		_expect(competitiveness == Vector3i(2, 1, 6), "Research-first technical competitiveness is incorrect.")
...
func _verify_scale_first(core: SimulationCore, state: GameState) -> void:
	var advanced: SimulationOperationResult = _advance_until_boundary(
		core,
		state,
		[_scale_command(state, 0)]
	)
	if advanced == null or not advanced.has_candidate_state():
		return
	var ended: GameState = advanced.candidate_state
	_expect(ended.cash_ledger.calculate_balance_musd() == 69, "Scale-first Cash is incorrect.")
	_expect(_category_total(ended, PROJECT_START_CATEGORY) == -30, "Scale-first Project cost is incorrect.")
	...
	_expect(_category_total(ended, BURST_COMPUTE_CATEGORY) == -24, "Scale-first burst compute cost is incorrect.")
	...
	_expect(
		_count_trace_events(advanced.trace, PostComputeContractCostsRule.EVENT_ID) == 6,
		"Scale-first did not post each active compute contract in each Month Step."
	)


func _verify_application_first(core: SimulationCore, state: GameState) -> void:
	var commit: SimulationOperationResult = _commit(core, state, [_coding_agent_command(state, 0)])
	...
	_expect(month_one.has_candidate_state(), "Application-first Month Step 1 has no candidate Game State.")
	if not month_one.has_candidate_state():
		return
	_expect(
		not month_one.candidate_state.company.applications.has(CODING_AGENT_APP_ID),
		"The Coding Agent became active before Month Step 2."
	)
	_expect(
		not _has_trace_event(month_one.trace, PostApplicationRevenueRule.EVENT_ID),
		"Month Step 1 posted Coding Agent Revenue."
	)
	...
	if not month_two.has_candidate_state():
		return
	_expect(
		month_two.candidate_state.company.applications.has(CODING_AGENT_APP_ID),
		"The Coding Agent Project did not create the Coding Agent in Month Step 2."
	)
	_expect(
		_revenue_payload_int(month_two.trace, &"customer_contract_count") == 12,
		"Month Step 2 demand is incorrect."
	)
	_expect(_revenue_payload_int(month_two.trace, &"revenue_musd") == 12, "Month Step 2 Revenue is incorrect.")
	_expect(
		month_two.candidate_state.company.applications[CODING_AGENT_APP_ID].active_customer_contract_count == 12,
		"Month Step 2 did not write the customer-contract count."
	)
	_expect(
		month_two.candidate_state.company.compute_capacity_unit_months == 70,
		"Month Step 2 inference reduced Compute Capacity."
	)
	...
	_expect(ended.cash_ledger.calculate_balance_musd() == 101, "Application-first Cash is incorrect.")
	_expect(_category_total(ended, PROJECT_START_CATEGORY) == -40, "Application-first Project cost is incorrect.")
	...
	_expect(_category_total(ended, REVENUE_CATEGORY) == 18, "Application-first Revenue is incorrect.")
...
func _verify_hybrid(core: SimulationCore, state: GameState) -> void:
	...
	if advanced == null or not advanced.has_candidate_state():
		return
	var ended: GameState = advanced.candidate_state
	_expect(ended.cash_ledger.calculate_balance_musd() == 36, "Hybrid Cash is incorrect.")
	_expect(_category_total(ended, PROJECT_START_CATEGORY) == -105, "Hybrid Project cost is incorrect.")
	...
	_expect(_category_total(ended, REVENUE_CATEGORY) == 18, "Hybrid Revenue is incorrect.")
	_expect(ended.company.models.has(RESEARCH_MODEL_ID), "Hybrid did not release the completed Model.")
	_expect(
		ended.company.applications[CODING_AGENT_APP_ID].supporting_model_id == STARTING_MODEL_ID,
