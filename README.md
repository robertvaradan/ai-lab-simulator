...
The production campaign presents the authored campus blockout and laboratory stage scenes. The SDF capture harness remains a separate render proof. It renders internally at 640×360 and scales to a 1920×1080 viewport.
...
- render-on-change compute dispatch (harness 640×360 scaled to 1920×1080);
...
- `game/host/sdf_campus_presenter.gd`: SDF capture presenter. The campaign HQ World does not use this presenter.
- `game/host/campus_visual_presenter.gd`: production campaign campus presenter. It selects laboratory stages from Simulation Core state.
